...
const StakePanel = ({
  ...
  const { data: flexBalance, refetch: refetchBalance } =
    useGetBalanceOfFLEX();
  ...
  // ✅ NEW: percentage buttons logic
  const setAmountByPercent = (percent) => {

    setSelectedPercent(percent);
    if (!flexBalance) return;

    const valueWei = (flexBalance * BigInt(percent)) / 100n;
    const value = formatEther(valueWei);
    ...
    >
      ...
      >
        ...
        <Typography color={grey[400]} mt={1} fontSize={13}>
          Enter stake name, amount and days.{" "}
          <span style={{ color: blue[100], fontWeight: 600 }}>
            Minimum
          </span>{" "}
          staking days is 7 and{" "}
          <span style={{ color: blue[100], fontWeight: 600 }}>
            maximum
          </span>{" "}
          is 3653.
        ...
        >
          <Typography color={grey[50]} fontSize={15} fontWeight={600}>
            FLEX Balance:&nbsp;
            <span style={{ color: blue[100], fontWeight: 600 }}>
              {flexBalance &&
                Number(formatEther(flexBalance)).toLocaleString("en-US", {
            ...
            {[25, 50, 75, 100].map((p) => (
              <Button
                ...
                sx={{
                  ...
                  fontSize: 12,
                  fontWeight: 600,
                  color: selectedPercent === p ? grey[900] : blue[100],
                  border: `1px solid ${borderColor}`,
                  borderRadius: 1,
                  textTransform: "none",
                  background: selectedPercent === p ? blue[100] : 'transparent',
                  px: 1,
                }}
              >
                {p}%
